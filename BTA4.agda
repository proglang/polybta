...
--module "EvaSource"
--note: it includes
--a)two-level type interpreter [ATInt];
--b)weaking of context of the target term [ATInt];
--c)environment [AEnv] for two-level type variables;
--d)weaking of context of the environment;
--e)lookup function [lookup];
--f)partial evaluator of two-level type terms [pe].
-------------------
module EvaSource where
