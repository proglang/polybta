-- The two-level version of Base and the partial evaluator. There is
-- one specialty: the language includes a construct `Lift' that marks
-- a subterm to be residualized by the partial evaluator. Not all
-- terms can be lifted; the `Liftable' predicate determines which can.

-- TODO: `Lift' should probably called `Reify', following TDPE jargon
-- TODO: Clarify again why Lift is restricted to Liftable
