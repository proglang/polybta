...
projP2D : ∀ {A : AType} {Δ'} {Δ : ACtx} {Γ : Ctx} {aen : AEnv Γ Δ}

            → Δ' ↝ Δ   
            → aexp (PVar'' Δ') A  
            → AExp Δ A
projP2D {Δ' = Δ'} {Δ = Δ} shift (Var x) = {!x !}
projP2D _ (SCst x) = AInt x
projP2D {AInt} {Δ} {Γ} {aen} shift (SAdd e e₁) 
     = AAdd (projP2D {AInt} {Δ} {Γ} {aen} shift e) (projP2D {AInt} {Δ} {Γ} {aen} shift e₁)
projP2D {AFun A B} {Δ'} {Δ} {Γ} {aen} shift (SLam x) 
        = ALam {!!}
{-
ALam {α₁ = A} {α₂ = B} 
...
produceΔ : ∀ {A : AType} {var : AType → Set} → aexp var A → ACtx
produceΔ {A} {var} (Var x) = A ∷ []
produceΔ (SCst x) = {!!}
produceΔ (SAdd e e₁) = {!!}
produceΔ (SLam x) = {!!}
produceΔ (SApp e e₁) = {!!}
