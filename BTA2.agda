...
env↑ _ env[] = env[]
env↑ Γ↝Γ' (env:: {α = α} x env) = env:: (int↑ {α} Γ↝Γ' x) (env↑ Γ↝Γ' env)
...
lookup () env[]
lookup hd (env:: x env) = x
lookup (tl idx) (env:: x env) = lookup idx env
...
pe {Γ = Γ} env (Lam {α₁} {α₂} S prf exp) =
  λ {Γ'} (prf₁ : Γ ↝ Γ') (v : ATInt Γ' α₂) → pe (env:: v (env↑ prf₁ env)) exp
...
pe {Γ = Γ} env (Lam {.(Ann D σ₁)} {.(Ann D σ₂)} D (wf-fun _ _ prf-1 prf-2) e)
  | is-dyn σ₁ | is-dyn σ₂ =
  ELam (pe (env:: (EVar hd) (env↑ ((strip' σ₂) ↝-∷ Γ) env)) e)
